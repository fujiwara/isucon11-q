...
func postInitialize(c echo.Context) error {
	...
	for rows.Next() {
		...
		var tm time.Time
		var trueCount int
		err = rows.Scan(&uuid, &tm, &trueCount)
		if err != nil {
			c.Logger().Errorf("db error : %v", err)
			return c.NoContent(http.StatusInternalServerError)
		}

		rdb.Set(nilCtx, fmt.Sprintf("latest_condition:%s", uuid), fmt.Sprintf("%d|%d", tm.Unix(), trueCount), 0)
	}
	rows.Close()

	_, err = db.Exec("UPDATE isu SET image = NULL")
	if err != nil {
		c.Logger().Errorf("db error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
